...
class Puzzle8_Problem:
    ...
    #possible moved depending on the current position.
    valid_moves=[
         [NO_MOVE,3,NO_MOVE,1],#possible moves for current position at 0 and so on
		 [NO_MOVE,4,0,2],
		 [NO_MOVE,5,1,NO_MOVE],
		 [0,6,NO_MOVE,4],
		 [1,7,3,5],
    ...
    def h_function(self,st):
        man_dist=0
        for i in range(1,9):
            diff=abs(st.index(i) - self.end_state.index(i))
            man_dist += int(diff/3) + int(diff%3)
        return man_dist
...
class TreeNode:
    ...
    def __init__(self, problem, state, parent=0,g_value=0):
        ...
        self.parent = parent
        self.g=g_value
        self.h=problem.h_function(self.state)
